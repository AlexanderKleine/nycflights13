...
```{r}
weather %>%
  select(origin, temp, wind_speed, visib) %>%
  filter(origin == "JFK") %>%
  summarise(avg_temp = mean(temp), avg_wind_speed = mean(wind_speed, na.rm = TRUE), avg_visib = mean(visib), origin = max(origin))
...
ggscatter(Question3, x = "avgde", y = "avgd", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "delay", ylab = "distance")

ggscatter(flights, x = "arr_delay", y = "distance", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "delay", ylab = "distance")
...
Question4 <- flights %>%
  ...
  summarise(visib = mean(visib), temp = mean(temp), precip = mean(precip), del = mean(dep_delay))


ggscatter(Question4, x = "del", y = "visib", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
...
The follow loop 
Question1answer <- Qestion1 %>%
  mutate(size_group = if (seats %in% 2:50) { 
    "Small"
  } else if (seats %in% 51:250) {
   "Medium"
  } else {
    "Larg"
  }
...
is giving that waring: 
**the condition has length > 1 and only the first element will be used**
The mutate function assigned the value "Small" to all values of $seats
